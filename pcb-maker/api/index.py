...
# ===================================================================
# ALLEGRO X -- VERCEL SERVERLESS KNOWLEDGE ENGINE v4.0
# Complete circuit templates + PCB knowledge base
# ===================================================================
...
def fuzzy_match(prompt):
    ...
    for key, tmpl in CIRCUIT_TEMPLATES.items():
        score = sum(len(kw) * 2 for kw in tmpl["keywords"] if kw in prompt)
        if score > best_score:
...
def knowledge_search(query):
    ...
    best_entry, best_score, best_category = None, 0, None
    for cat_key, category in PCB_KNOWLEDGE.items():
        for ek, entry in category["entries"].items():
            score = sum(len(kw) * 3 for kw in entry.get("keywords", []) if kw in query)
            if score > best_score:
                best_score, best_entry, best_category = score, entry, category["title"]
...
def generate():
    ...
    if key and score > 0:
        ...
        return jsonify({
            ...
            "nets": tmpl["nets"], "component_count": len(tmpl["components"]),
            "net_count": len(tmpl["nets"]),
        })
    return jsonify({"success": False, "message": f"Could not match '{prompt}'. Try: '555 timer', 'LED', '5V power supply'."})
...
def list_templates():
    ...
    for key, tmpl in CIRCUIT_TEMPLATES.items():
        ...
        presets.append({"id": key, "name": name, "description": desc, "component_count": len(tmpl["components"])})
...
def get_knowledge():
    result = {}
    for cat_key, category in PCB_KNOWLEDGE.items():
        entries = [{"id": ek, "name": e["name"], "info": e["info"], **({k: e[k] for k in ["specs"] if k in e})} for ek, e in category["entries"].items()]
...
def ask_knowledge():
    ...
    if entry and score > 0:
        resp = {"success": True, "category": category, "topic": entry["name"], "answer": entry["info"]}
        if "specs" in entry:
            resp["specs"] = entry["specs"]
        return jsonify(resp)
    return jsonify({"success": False, "message": f"No knowledge match for '{query}'. Try: FR-4, HDI, reflow soldering, IPC classes."})
